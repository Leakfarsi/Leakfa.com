...
function search($hash){
    ...
    global $db;
    $stmt = $db->prepare("SELECT `source` FROM `breach_phone` WHERE `hash`=:hash");
    $stmt->execute([
        'hash' => $_GET['hash']
    ]);
...
function subscribe($name, $email, $hash){
    ...
    if (is_account_exist($email)){
        ...
        if (is_account_verify($email)){
            ...
            $res['error'] = 'this email has subscribed to the leaked message, but has not verified the E-mail, please go to your email to confirm.';
        }
    }else{
